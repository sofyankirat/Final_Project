...
class NLPController(BaseController):
    ...
    def __init__(self, vectordb_client, generation_client, 
                 embedding_client, template_parser, provider):
        super().__init__()
    ...
    async def index_into_vector_db(self, project: Project, chunks: List[DataChunk],
        ...
        # step 2.1: check embedding result
        if isinstance(vectors, ResponseSignal):
            return vectors
    ...
    async def answer_rag_question(self, project: Project, query: str, limit: int = 3, chat_history: list = None):
        
        answer, full_prompt, parsed_chat_history = None, None, None
        ...
        documents_prompts = "\n".join([
            self.template_parser.get("rag", "document_prompt", {
                    ...
                    "chunk_metadata": doc.metadata if hasattr(doc, 'metadata') and doc.metadata else {}
            })
        ...
        # Parse chat_history passed from frontend
        if chat_history:
            for message in chat_history:
                ...
                # Map standard roles to provider-specific roles
                if msg_role == "user":
                    p_role = self.generation_client.enums.USER.value
                elif msg_role in ["bot", "assistant"]:
                    p_role = self.generation_client.enums.ASSISTANT.value
                else:
                    continue
