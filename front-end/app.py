...
def debug_email():
    ...
    if not brevo_api_key and email_pwd:
        pwd_clean = email_pwd.strip().replace('\r', '').replace('\n', '').replace(' ', '').replace('\t', '')
        if pwd_clean.startswith("xkeysib-") or len(pwd_clean) > 40:
            brevo_api_key = pwd_clean

    email_addr = os.getenv('EMAIL_ADDRESS', '')
    sender_email = normalize_email_address(email_addr)
    
    logs = []
    logs.append(f"BREVO_API_KEY configured: {bool(brevo_api_key)}")
    if brevo_api_key:
        logs.append(f"BREVO_API_KEY prefix: {brevo_api_key[:15]}...")
        logs.append(f"BREVO_API_KEY length: {len(brevo_api_key)}")
    logs.append(f"EMAIL_ADDRESS env: {email_addr}")
    logs.append(f"EMAIL_PASSWORD env length: {len(email_pwd)}")
    logs.append(f"Normalized Sender Email: {sender_email}")
    logs.append(f"Recipient Email: {recipient}")
    
    # Try sending via Brevo API
    headers = {
        "accept": "application/json",
        "api-key": brevo_api_key if brevo_api_key else '',
        "content-type": "application/json"
    }
    payload = {
        "sender": {
        ...
        "htmlContent": "<h1>This is a debug test email</h1>"
    }
    ...
    try:
        ...
        brevo_response = res.text
    except Exception as e:
        brevo_response = traceback.format_exc()
    ...
    try:
        ...
        if os.path.exists(log_path):
            with open(log_path, 'r', encoding='utf-8') as f:
                logs_file_content = f.read()
        else:
            logs_file_content = "email_logs.txt does not exist yet."
    except Exception as file_err:
        logs_file_content = f"Error reading logs file: {str(file_err)}"
        
    return jsonify({
        'debug_info': logs,
        'logs_file_content': logs_file_content
    })
